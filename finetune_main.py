...
from datasets import faced_dataset, seedv_dataset, physio_dataset, shu_dataset, isruc_dataset, chb_dataset, \
    speech_dataset, mumtaz_dataset, seedvig_dataset, stress_dataset, tuev_dataset, tuab_dataset, bciciv2a_dataset, chongqing_dataset
...
from models import model_for_faced, model_for_seedv, model_for_physio, model_for_shu, model_for_isruc, model_for_chb, \
    ...
    model_for_bciciv2a, model_for_chongqing


def main():
    parser = argparse.ArgumentParser(description='Big model downstream')
    parser.add_argument('--seed', type=int, default=3407, help='random seed (default: 0)')
    parser.add_argument('--cuda', type=int, default=0, help='cuda number (default: 1)') # changed from 1 to 0
    parser.add_argument('--epochs', type=int, default=30, help='number of epochs (default:35)') 
    parser.add_argument('--batch_size', type=int, default=64, help='batch size for training (default: 32)')
    parser.add_argument('--lr', type=float, default=3e-5, help='learning rate (default: 1e-4)')
    parser.add_argument('--weight_decay', type=float, default=5e-3, help='weight decay (default: 1e-2)')
    ...
    parser.add_argument('--downstream_dataset', type=str, default='CHONGQING',
                        help='[FACED, SEED-V, PhysioNet-MI, SHU-MI, ISRUC, CHB-MIT, BCIC2020-3, Mumtaz2016, '
                             'SEED-VIG, MentalArithmetic, TUEV, TUAB, BCIC-IV-2a, CHONGQING]')
    parser.add_argument('--datasets_dir', type=str,
                        default='processed/',
                        help='datasets_dir')
    parser.add_argument('--num_of_classes', type=int, default=1, help='number of classes')
    parser.add_argument('--model_dir', type=str, default='new_model/models_weights', help='model_dir') 
    """############ Downstream dataset settings ############"""

    parser.add_argument('--num_workers', type=int, default=16, help='num_workers')
    parser.add_argument('--label_smoothing', type=float, default=0, help='label_smoothing') # disabled label smoothing for regression, original was 0.1
    parser.add_argument('--multi_lr', type=bool, default=False,
                        help='multi_lr')  # set different learning rates for different modules --- default was True 
    ...
    if params.downstream_dataset == 'FACED':
        ...
        t = Trainer(params, data_loader, model)
        t.train_for_binaryclass()
    elif params.downstream_dataset == 'CHONGQING':
        load_dataset = chongqing_dataset.LoadDataset(params)
        data_loader = load_dataset.get_data_loader()
        model = model_for_chongqing.Model(params)
        t = Trainer(params, data_loader, model)
        t.train_for_regression()
